...
const MOCKS: Mock[] = [
  {
    path: createPathRegExp("/movies/:id"),
    mockData: (param) => {
      const options = {
      ...
      return options[param as keyof typeof options];
    },
  },
  {
    path: createPathRegExp("/movies"),
    mockData: () => {
      return [
        {
          id: "e80d5a37-620e-4be2-92b9-fb1f5262494f",
  ...
  {
    path: createPathRegExp("/options/:id"),
    mockData: (param) => {
      const options = {
        S: {
          configs: [
...
export function mockingInterceptor(
  req: HttpRequest<unknown>,
  next: HttpHandlerFn
): Observable<HttpEvent<unknown>> {
  const mock = MOCKS.find((m) => m.path.test(req.url));
  if (mock) {
